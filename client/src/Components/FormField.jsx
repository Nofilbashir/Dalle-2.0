...
const FormField = ({labelName,type,name,placeholder,value,handleChange,isSurprizeMe,handleSurprizeMe}) => {
  return (
    <div>
      <div className='flex items-center gap-2 mb-2'>
          <label htmlFor={name} className="block text-sm font-medium text-gray-900">{labelName}</label>
          {isSurprizeMe && (
            <button type='button' onClick={handleSurprizeMe} className="font-semibold text-xs bg-[#ECECF1] py-1 px-2 rounded-[5px] text-black">Surprize me</button>
          )}
      </div>
